    ...
    <>
      <div style={{ background: "white", borderBottom: "1px solid #EBEBEB" }} className="sticky top-0 z-10 px-4 sm:px-6 py-3">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Active filter chips */}
          {currentListingType && <span style={chipStyle}>{currentListingType === "rent" ? "For rent" : "For sale"}</span>}
          {currentPropertyType && <span style={chipStyle}>{currentPropertyType.charAt(0).toUpperCase() + currentPropertyType.slice(1)}</span>}
          {selectedAreas.length > 0 && <span style={chipStyle}>{selectedAreas.length === 1 ? selectedAreas[0] : `${selectedAreas.length} areas`}</span>}
          {currentMaxPrice && <span style={chipStyle}>Up to {Number(currentMaxPrice).toLocaleString()} KWD</span>}

          {/* Right side */}
          <div className="flex gap-2 ml-auto">
            <button onClick={toggleMap} style={btnStyle}>
              {showMap ? (
                <><svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 10h16M4 14h16M4 18h16"/></svg>List</>
              ) : (
                <><svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7"/></svg>Map</>
              )}
            </button>
            <a href="/listings/new" style={{ background: "#FF385C", color: "white", border: "none", fontSize: "13px", borderRadius: "24px", padding: "7px 16px", fontWeight: 600, whiteSpace: "nowrap" as const, textDecoration: "none" }}>+ Post</a>
          </div>
