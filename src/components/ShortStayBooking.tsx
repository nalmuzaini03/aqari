...
export default function ShortStayBooking({ phoneNumber, title, pricePerNight }: Props) {
  ...
  function handleWhatsApp() {
    ...
    const message = checkIn && checkOut
      ? `Hi, I'd like to book "${title}" from ${checkIn} to ${checkOut} (${nights} night${nights !== 1 ? "s" : ""}). Is it available?`
  ...
  return (
    <div style={{ border: "1px solid #EBEBEB", borderRadius: "16px", padding: "24px", marginBottom: "16px" }}>
      <p style={{ fontSize: "13px", color: "#717171", fontWeight: 600, letterSpacing: "0.5px", marginBottom: "16px" }}>SELECT DATES</p>

      <div className="grid grid-cols-2 gap-3 mb-4">
        <div style={{ border: "1px solid #DDDDDD", borderRadius: "8px", padding: "12px 16px" }}>
          <p style={{ fontSize: "10px", fontWeight: 700, color: "#222", letterSpacing: "0.5px", marginBottom: "4px" }}>CHECK-IN</p>
          <input
            type="date"
            value={checkIn}
            min={today}
        ...
        <div style={{ border: "1px solid #DDDDDD", borderRadius: "8px", padding: "12px 16px" }}>
          <p style={{ fontSize: "10px", fontWeight: 700, color: "#222", letterSpacing: "0.5px", marginBottom: "4px" }}>CHECK-OUT</p>
          <input
            type="date"
            value={checkOut}
            min={checkIn || today}
      ...
      {nights > 0 && (
        <div style={{ background: "#F7F7F7", borderRadius: "12px", padding: "16px", marginBottom: "16px" }}>
          <div className="flex justify-between mb-2">
            <p style={{ fontSize: "14px", color: "#222" }}>{pricePerNight} KWD × {nights} night{nights !== 1 ? "s" : ""}</p>
            <p style={{ fontSize: "14px", color: "#222" }}>{total.toLocaleString()} KWD</p>
          </div>
          <div style={{ borderTop: "1px solid #EBEBEB", paddingTop: "12px", marginTop: "8px" }} className="flex justify-between">
            <p style={{ fontSize: "15px", fontWeight: 700, color: "#222" }}>Total</p>
            <p style={{ fontSize: "15px", fontWeight: 700, color: "#222" }}>{total.toLocaleString()} KWD</p>
          </div>
        </div>
      )}
      ...
      >
        ...
        {checkIn && checkOut ? `Request booking via WhatsApp` : "Contact via WhatsApp"}
      </button>

      {checkIn && checkOut && (
        <p style={{ fontSize: "12px", color: "#717171", textAlign: "center", marginTop: "10px" }}>
          Your dates will be sent automatically in the WhatsApp message
        </p>
      )}
    </div>
  )
